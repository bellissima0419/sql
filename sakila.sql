...
FROM
    actor
WHERE
    (first_name , last_name) = ('GROUCHO' , 'WILLIAMS');
UPDATE actor
SET
    first_name = 'HARPO'
WHERE
...
SELECT
    title, COUNT(title) AS number_of_copies
FROM
    film
        INNER JOIN
...
SELECT
    store.store_id, SUM(amount) AS 'Total Rental Amount($)'
FROM
    payment
        JOIN
